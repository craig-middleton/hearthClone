...
_Last updated: 2026-08-18 (session 30 — `MinionView` root/`ArtworkImage` resized to 160x220 to match `CardView`, fixing the board card size-mismatch bug; spell-cast animation's travel-effect + target-reaction vertical slice built and confirmed via playtest, face-only)_
...
8. **`HandDisplay.RenderHand` and `BoardDisplay.RenderBoard` are structurally identical by design** — same guards in the same order. A change to one must be mirrored in the other. **Deliberate exception since session 30**: `HandDisplay.RenderHand`'s click callback is `Action<CardData, CardView>` (the spell-animation sequencer needs the clicked `CardView`'s position — see #15); `BoardDisplay.RenderBoard`'s is still plain `Action<Minion>`. This is a known, accepted asymmetry, not a missed mirror — a future minion-summon-animation feature will hit the same need and should extend `BoardDisplay`'s callback the same way rather than rediscovering why.
...
| `FaceView.cs` | `Scripts/UI/` | Health + mana display, doubling as the face attack target. `SetPlayer(Player, Action<Player>)` writes `"{PlayerName}: {Health} HP\nMana: {CurrentMana}/{MaxMana}"` and wires a `Button`; guards `playerData`. Idle animation in `Update()`: sine scale pulse + sway, base captured on first `SetPlayer()`, four tunable fields. Constraint 9. `PlayDamageReaction()` (session 30) flashes `avatarImage.color` to `damageFlashColor` and back over `reactionDuration`, called by `SpellAnimationSequencer` on impact — deliberately color-only, not position/scale, so it doesn't fight `Update()`'s per-frame sway/breathe. |
| `SpellAnimationSequencer.cs` | `Scripts/UI/` | Spell-cast animation, face-only vertical slice (travel + impact reaction; lift/zoom and discard-off not built — Constraint 15, Next Steps). `PlayTravelAndReaction(Vector3 sourceWorldPosition, Transform targetView, bool isDamage)` spawns a plain runtime `Image` (no prefab/particle asset), lerps it to `targetView` over `travelDuration`/`travelCurve` (re-sampling `targetView.position` every frame since `FaceView` sways concurrently), destroys it, then calls `targetView.GetComponent<FaceView>().PlayDamageReaction()` when `isDamage`. No heal/buff branch yet — no `CardEffect` subtype to key off. Called from `EffectTester.TriggerSpellAnimation`/`ResolveViewTransform`. |
...
- ✅ **Confirmed via playtest (session 30)**: spell-cast animation's travel-effect + target-reaction vertical slice — playing Fireball/Arcane Bolt against the opponent's face shows the projectile traveling from card to face and a red flash on impact. Face-only; lift/zoom and discard-off not built yet (Next Steps).
