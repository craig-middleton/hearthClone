...
## Verified Working
Confirmed end-to-end: playing `TestCard_Fireball` through `EffectTester` in Play mode correctly triggers `Effect_Deal3Damage`, logging starting health and post-damage health to the Console. Full pipeline (`CardData` → `CardEffect` → `Execute` → `Target`) works.

## Current Blocker / Last Thing Worked On
None — last blocker (circular assembly reference from `EffectTester` living in `Core`) resolved by moving it to `Scripts/Cards/`. Ready to move on to real game state (`Player`/`Board`).
...
## Next Steps (in order)
1. Build real `Player` and `Board` classes in `Core` — will replace placeholder `Target`/`GameContext` with real game state (hand, mana, board minions, turn tracking).
2. Build `TurnManager` (turn order, mana refill each turn, win/loss conditions).
3. Build basic AI opponent logic.
4. Build Card UI (hand display, drag-and-drop to board).
