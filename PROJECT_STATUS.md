...
| `PlayerHand.cs` | `Scripts/Cards/` | Wraps a `Core.Player` with `Deck`/`Hand`. `Shuffle()`, `DrawOpeningHand(int)`, `AddCardToHand()`, `MulliganCard()`. **Pasted in and compiling this session**: `DrawCard()` now deals escalating fatigue damage (`CorePlayer.FatigueDamage++` then `TakeDamage()`) when the deck is empty instead of doing nothing; burns (discards) a newly-drawn card if `Hand.Count >= MaxHandSize` (10); `PlayCard()` blocks Minion plays entirely if `BoardMinions.Count >= MaxBoardSize` (7), checked before mana/hand are touched. **New this session (Taunt)**: `PlayCard()` now passes `card.hasTaunt` through to the `Minion` constructor when summoning. Neither the fatigue/cap code nor Taunt have been stress-tested yet (fatigue/caps need a long game to actually trigger; Taunt itself is fully verified — see below). |
...
- Neither fatigue (needs a fully emptied deck) nor the board cap (needs 7 minions on one side) came up naturally in this session's shorter Taunt-focused playtest. Worth a longer test session at some point to confirm both actually fire correctly when triggered.
